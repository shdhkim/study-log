...
- **정의**: 두 변수의 선형 관계(같이 커지거나 작아지는 경향)를 측정
- **수식**:  
  \[
...
- **정의**: 공분산을 정규화한 값, 두 변수 간 선형 관계의 강도 측정
- **수식**:  
  \[
...
- **특징**:
  - 단위 영향 없음
  - 1: 완전한 양의 선형 관계, 0: 무관, -1: 완전한 음의 관계
...
- **정의**: 두 점 사이의 절댓값 거리 (길거리 거리 개념)
- **수식**:  
  \[
  \text{d}(A, B) = \sum |a_i - b_i|
  \]
- **범위**: \(0 ~ +\infty\)
- **특징**:
  - 유클리드보다 이상치에 덜 민감
