...
# 📌 최근접 이웃 탐색 알고리즘 비교 – ANN vs KNN vs Brute-force

벡터나 고차원 데이터에서 유사한 데이터를 찾는 대표적인 알고리즘들을 비교합니다.
...
|-------------|----------------------------|-----------------------------|-----------------------------------|
| 목적        | 정확한 이웃 찾기           | 분류/회귀                   | 빠른 근사 이웃 찾기               |
| 계산 방식   | 모든 점과 거리 계산         | 모든 점과 거리 계산         | 일부 후보만 빠르게 계산           |
| 정확도      | 최고                        | 높음                        | 근사 (속도와 정확도 트레이드오프) |
| 속도        | 느림                        | 느림 (소규모는 빠름)        | 매우 빠름                         |
| 데이터 규모 | 소규모 적합                 | 소~중규모                   | 대규모 적합 (수백만 이상)         |
| 사용 예     | 실험, 검증용, 정확성 필요시 | 간단한 분류/회귀            | 벡터 검색, 추천 시스템, 이미지 검색 |
