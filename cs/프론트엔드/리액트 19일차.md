...
근데 MyInput이 **함수 컴포넌트**면, 기본적으로 ref를 props처럼 받을 수 없다.  
이때 forwardRef를 쓰면:

- ref를 **두 번째 인자**로 받을 수 있음
- 그 ref를 내부 DOM에 붙이거나
- useImperativeHandle로 “내가 공개할 것만” 공개할 수도 있음

---

## 2. useImperativeHandle는 뭐함?

useImperativeHandle(ref, createHandle, deps) 는:

- 부모가 보게 될 ref.current의 모양(내용)을 **내가 직접 정의**하게 해준다.
...
##  1-A. forwardRef로 DOM ref를 그대로 전달
...
### 동작
- 부모 inputRef.current는 실제 <input> DOM
- 그래서 .focus()를 직접 호출 가능

---

##  1-B. (권장 패턴) useImperativeHandle로 “focus만” 공개
...
### 포인트
- 부모는 이제 ref.current를 통해 **DOM 자체가 아니라 API 객체**를 받는다.
...
### 포인트
- useImperativeHandle의 deps에 [name, age]를 둔 이유:
  - 부모가 보는 validate()가 최신 name/age를 반영하게 만들기 위함
