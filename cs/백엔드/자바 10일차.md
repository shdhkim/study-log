...
## 12. Stream.map은 왜 Function을 요구하는가
...
- `map`은 **Function을 파라미터로 요구**한다
- 요소 하나를 다른 값으로 **변환(transform)** 하는 연산이다

즉, map은 다음 질문에 답하는 연산이다.

> “각 요소를 무엇으로 바꿀 것인가?”

---

## 13. map에서 Function이 사용되는 방식
...
→ **Function의 전형적인 사용 패턴**

---

## 14. map과 sorted의 역할 차이

| 연산 | 요구 타입 | 핵심 역할 |
|---|---|---|
| map | Function<T, R> | 변환 |
| sorted | Comparator<T> | 비교 |
...
- sorted는 Comparator 필요
- `(T) -> R` 형태는 Comparator가 될 수 없음

---

## 15. 메서드 레퍼런스도 동일하게 Function으로 해석됨

```java
map(User::getAge);          // Function<User, Integer>
...
- 메서드 레퍼런스 자체에는 타입이 없다
- **Function을 요구하는 자리에 놓였기 때문에 Function으로 해석**
- 해석 주체는 항상 컴파일러
...
map은 변환 연산이기 때문에 Function을 요구한다.  
람다와 메서드 레퍼런스는 그 자리에 맞춰 Function으로 해석된다.
